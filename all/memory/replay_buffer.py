...
class ExperienceReplayBuffer(ReplayBuffer):
    ...
    def store(self, states, actions, rewards, next_states):
        ...
        for state, action, reward, next_state in zip(states, actions, rewards, next_states):
            if state is not None and not state.done:
                self._add((state.to("cpu"), action.to("cpu"),
                           reward.to("cpu"), next_state.to("cpu")))
    ...
    def _reshape(self, minibatch, weights):
        ...
        actions = torch.tensor([sample[1]
                                for sample in minibatch], device=self.device)
        rewards = torch.tensor(
            [sample[2] for sample in minibatch], device=self.device).float()
...
class PrioritizedReplayBufferWithExpert(PrioritizedReplayBuffer):
    '''PrioritizedReplayBuffer with expert demonstrations'''
    ...
    def __init__(self,
        ...
        super().__init__(buffer_size, alpha, beta, epsilon, device)
        for data in expert_buffer:
            super().store(*data)
        assert len(
            expert_buffer) < buffer_size, "Expert buffer size exceeds buffer size"
        self.expert_size = len(expert_buffer)
        self.pos = int(self.expert_size)
    ...
    def _add(self, sample):
        if len(self.buffer) < self.capacity:
            ...
            self.buffer[self.pos] = sample
        self.pos = self.expert_size + (self.pos - self.expert_size + 1)\
            % (self.capacity - self.expert_size)
