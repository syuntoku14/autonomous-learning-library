...
class C51(Agent):
    ...
    def act(self, states, rewards):
        print("---------------------")
        print("current_size: ", len(self.replay_buffer))
        if self._states is None and self._actions is None:
            self._states = [None for _ in range(len(states))]
            self._actions = [None for _ in range(len(states))]
        for _state, _action, reward, state in zip(self._states, self._actions, rewards, states):
            self.replay_buffer.store(_state, _action, reward, state)
        print("current_size: ", len(self.replay_buffer))
        self._train()
        self._states = states
        self._actions = self._choose_action(states)
        return self._actions

    def _choose_action(self, states):
        if self._should_explore():
            return torch.randint(
                self.q_dist.n_actions, (len(states),), device=self.q_dist.device
            )
        return self._best_actions(states)
